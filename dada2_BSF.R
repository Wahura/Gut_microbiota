...
sdata <- sdata %>% remove_rownames %>% column_to_rownames(var="Sample.id")
...
ntaxa(physeq0)
filtered_physeq <- prune_taxa(taxa_sums(physeq0) > 5, physeq0)
filtered_physeq
...
blast_abundance <- tax_table[,c(1, 9:47)]
...
Featured_table <- silva_blast[,c(7,9:64)]
#grouping the data
...
to_represent <- c("Lactobacillus", "Snodgrassella", "Saccharibacter","Bifidobacterium", "Neokomagataea", "	Saccharibacter","Bombella","	Ameyamaea",
                  "Nguyenibacter", "Zymobacter", "Acinetobacter", "Gluconacetobacter", "Enterococcus") 
...
vd <- venn.diagram(x = list("HA" = rownames(HA),
                            "HG" = rownames(HG),
                            "LG" = rownames(LG),
                             "LN" = rownames(LN),
                             "MB" = rownames(MB),
                             ...
                             "MFR" = rownames(MFR),
                             "DS" = rownames(DS)),
                   fill = brewer.pal(3, "Set2")[1:8], filename = NULL)
grid.draw(vd)
library(gplots)
vd <- list(HA, HG, LG, LN, MB, MFB, MFR, DS)
names(vd) = c("HA", "HG", "LG", "LN", "MB", "MFB", "MFR", "DS")
venn(vd)
...
taxonomy_table <- silva_blast[,c(1:8)]
features <-silva_blast[,c(1,9:64)]
...
plot_ordination(physeq3, ordu, color="species", shape = "species")

#alpha diversity estimation
physeq4 <- physeq3

#checking out the total read counts in the samples
reads <- sample_sums(physeq3)
reads

summary(sample_sums(physeq3))
...
rarefied <- rarefy_even_depth(physeq1, sample.size = 34750)
rarefied
...
shannon <- diversity[, c(1,6)]
colnames(shannon)<- c("sample", "shannon")

#creating a classification collumn for the various diets
shannon$sample_category <- (gsub('[0-9]', '', shannon$sample))
...
p <- ggplot(shannon, aes(x=sample_category, y=shannon)) + geom_boxplot(aes(fill = sample_category)) +
